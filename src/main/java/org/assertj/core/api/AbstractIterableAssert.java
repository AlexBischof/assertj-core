...
import org.assertj.core.api.filter.FilterOperation;
...
public abstract class AbstractIterableAssert<S extends AbstractIterableAssert<S, A, T>, A extends Iterable<? extends T>, T>
  ...
  public S filterOn(String propertyName, Object propertyValue) {
    return (S) Assertions.assertThat(Filters.filter((Iterable<A>) actual).with(propertyName, propertyValue).get());
  }

  public S filterOn(String propertyName, FilterOperation filterOperation) {
      Filters<A> filters = Filters.filter((Iterable<A>) actual).with(propertyName);
      filterOperation.apply(filters);
      return (S) Assertions.assertThat(filters.get());
  }
}
